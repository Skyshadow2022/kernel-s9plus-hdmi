...
void displayport_reg_set_video_bist_mode(u32 en)
{
	...
	/*
	 * Only toggle BIST_EN here. Clearing STRM_VALID_FORCE/CTRL on
	 * bist=0 blacks the live DECON→DP path (proven on PE/S9+).
	 * Live cutover re-asserts STRM_VALID_* after VIDEO_EN.
	 */
	displayport_write_mask(SST1_VIDEO_BIST_CONTROL, val, BIST_EN);
	if (en)
		displayport_write_mask(SST1_VIDEO_CONTROL, ~0,
...
void displayport_reg_set_stream_valid_force(u32 en)
{
	...
	displayport_write_mask(SST1_VIDEO_CONTROL, val,
			STRM_VALID_FORCE | STRM_VALID_CTRL);
}
