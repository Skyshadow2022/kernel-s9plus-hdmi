...
void decon_set_color_window(struct decon_device *decon, u32 color)
{
	...
	int win = decon->dt.dft_win;
	u32 shd;

	decon_to_psr_info(decon, &psr);

	/* Start DECON before queueing shadow updates or they never retire */
	if (!decon_reg_get_run_status(decon->id)) {
		decon_info("decon%d color_window: starting hw\n", decon->id);
		decon_reg_start(decon->id, &psr);
	}

	/* Unmask HW/SW trigger so VIDEO-mode shadow can retire on VSYNC */
	decon_reg_set_trigger(decon->id, &psr, DECON_TRIG_ENABLE);

	/* Wait out any stale shadow request first */
	shd = decon_read(decon->id, SHADOW_REG_UPDATE_REQ);
	if (shd) {
		decon_warn("decon%d color_window: stale shadow=0x%x, waiting\n",
				decon->id, shd);
		decon_reg_set_trigger(decon->id, &psr, DECON_TRIG_ENABLE);
	...
	memset(&win_regs, 0, sizeof(struct decon_window_regs));
	win_regs.wincon = wincon(0x8, 0xFF, 0xFF, 0xFF, DECON_BLENDING_NONE, win);
	win_regs.start_pos = win_start_pos(0, 0);
	win_regs.end_pos = win_end_pos(0, 0, lcd->xres, lcd->yres);
	...
	decon_info("decon%d solid color=0x%08x %dx%d win=%d run=%u shd=0x%x trig=0x%x\n",
			decon->id, color, lcd->xres, lcd->yres, win,
			decon_reg_get_run_status(decon->id),
			decon_read(decon->id, SHADOW_REG_UPDATE_REQ),
			decon_read(decon->id, HW_SW_TRIG_CONTROL));

	decon_reg_set_window_control(decon->id, win, &win_regs, true);
	/* Global+window: DP/VIDEO freerun paths sometimes ignore win-only req */
	decon_reg_update_req_window(decon->id, win);
	decon_reg_update_req_global(decon->id);
	decon_reg_set_trigger(decon->id, &psr, DECON_TRIG_ENABLE);

	if (decon_reg_wait_update_done_and_mask(decon->id, &psr, SHADOW_UPDATE_TIMEOUT) < 0) {
		shd = decon_read(decon->id, SHADOW_REG_UPDATE_REQ);
		decon_err("decon%d color_window shadow timeout shd=0x%x glob=0x%x path0=0x%x\n",
				decon->id, shd,
				decon_read(decon->id, GLOBAL_CONTROL),
				decon_read(decon->id, DATA_PATH_CONTROL_0));
		decon_reg_update_req_global(decon->id);
		decon_reg_set_trigger(decon->id, &psr, DECON_TRIG_ENABLE);
		decon_reg_wait_update_done_and_mask(decon->id, &psr, SHADOW_UPDATE_TIMEOUT);
	} else {
		decon_info("decon%d color_window shadow OK\n", decon->id);
	}
}
		...
		struct decon_win_config *config)
{
	...
	/*
	 * WIN/DMA are shared across DECONs. While DP (decon2) is ON, primary
...
static int decon_fb_alloc_memory(struct decon_device *decon, struct decon_win *win)
{
	...
	{
		unsigned int aw = lcd_info->xres;
		unsigned int ah = lcd_info->yres;

		/* DP fb is allocated once at probe (often 640x480 default).
		 * Pre-size for FHD so mirror/HWC can FBIOPUT up to 1080p.
		 */
		if (decon->dt.out_type == DECON_OUT_DP) {
			if (aw < 1920)
				aw = 1920;
