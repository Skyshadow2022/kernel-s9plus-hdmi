...
static int decon_displayport_set_lcd_info(struct decon_device *decon)
{
	...
	/*
	 * Stock Exynos DP uses HW_TRIG (TE from DPIF). SW_TRIG left DECON2
	 * without frame cadence → FIFO_UNDER_FLOW + shadow timeout (black).
	 */
	...
	/* Keep fb1 var in sync with sink — probe left 640x480 geometry */
	if (decon->dt.dft_win < MAX_DECON_WIN &&
	    decon->win[decon->dt.dft_win] &&
	    decon->win[decon->dt.dft_win]->fbinfo) {
		struct fb_info *fbi = decon->win[decon->dt.dft_win]->fbinfo;
		u32 w = decon->lcd_info->xres;
		u32 h = decon->lcd_info->yres;

		fbi->var.xres = w;
		fbi->var.yres = h;
		fbi->var.xres_virtual = w;
		fbi->var.yres_virtual = h * 2;
		fbi->var.bits_per_pixel = 32;
		fbi->var.width = decon->lcd_info->width;
		fbi->var.height = decon->lcd_info->height;
		fbi->var.xoffset = 0;
		fbi->var.yoffset = 0;
		fbi->fix.line_length = w * 4;
		decon_info("decon%d fb1 synced %dx%d smem=%u\n",
				decon->id, w, h, fbi->fix.smem_len);
	}
#else
