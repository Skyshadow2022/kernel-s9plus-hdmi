...
static int decon_set_win_info(struct fb_info *info)
{
	...
	/* DP: window must cover sink (lcd_info), not probe fb geometry */
	if (decon->dt.out_type == DECON_OUT_DP && decon->lcd_info) {
		out_w = decon->lcd_info->xres;
		out_h = decon->lcd_info->yres;
	} else {
		out_w = var->xres;
		out_h = var->yres;
	}
	...
	win_regs.end_pos = win_end_pos(0, 0, out_w, out_h);
	win_regs.pixel_count = (out_w * out_h);
	win_regs.whole_w = out_w;
	win_regs.whole_h = out_h;
	win_regs.offset_x = 0;
	win_regs.offset_y = 0;
...
int decon_pan_display(struct fb_var_screeninfo *var, struct fb_info *info)
{
	...
	/* DSI primary + DP external (hdmi-mirror / prefer_live fb1 path) */
	if (decon->dt.out_type != DECON_OUT_DSI &&
	    decon->dt.out_type != DECON_OUT_DP) {
		decon_warn("%s: decon%d unsupported on out_type(%d)\n",
	...
	switch (var->bits_per_pixel) {
		...
		case 24:
		case 32:
			/* Android screencap / mirror feed RGBA in memory */
			config.format = (decon->dt.out_type == DECON_OUT_DP) ?
	...
	config.blending = DECON_BLENDING_NONE;
	config.plane_alpha = 0xff;
	config.dpp_parm.addr[0] = info->fix.smem_start;
	config.idma_type = decon->dt.dft_idma;
	...
	/*
	 * DP uses VGF1 — scale src fb up to full sink. 1:1 left a partial
	 * window and wrong BW when var was still 640x480 vs 1080p lcd_info.
	 */
	if (decon->dt.out_type == DECON_OUT_DP && decon->lcd_info) {
		config.dst.x = 0;
		config.dst.y = 0;
	...
	/* Bandwidth before arming DMA (colormap path needs none) */
	if (decon->dt.out_type == DECON_OUT_DP && decon->bts.enabled &&
	    decon->bts.ops && decon->bts.ops->bts_calc_bw) {
		struct decon_reg_data bw_regs;

		memset(&bw_regs, 0, sizeof(bw_regs));
		bw_regs.num_of_window = 1;
		bw_regs.dpp_config[decon->dt.dft_win] = config;
		decon->bts.ops->bts_calc_bw(decon, &bw_regs);
		decon->bts.ops->bts_update_bw(decon, &bw_regs, 0);
		if (decon->bts.ops->bts_acquire_bw)
	...
	if (!sd || v4l2_subdev_call(sd, core, ioctl, DPP_WIN_CONFIG, &config)) {
	...
	/* DP needs trigger armed for VIDEO shadow retire (HW and/or SW+timer) */
	if (decon->dt.trig_mode == DECON_SW_TRIG ||
	    decon->dt.out_type == DECON_OUT_DP) {
		decon_reg_set_trigger(decon->id, &psr, DECON_TRIG_ENABLE);
		if (decon->dt.out_type == DECON_OUT_DP) {
			...
			/*
			 * Do not pulse live_trig around DMA pan — SW_TRIG storms
			 * collide with DPP fetch (DECON2 timeout + FIFO under).
			 * Video-mode + HW trig should scan the armed buffer.
			 */
			displayport_live_trig_stop();
		}
	}
err:
	/* DP/SW-trig: vsync wait often times out; don't block the cutover */
	if (decon->dt.out_type != DECON_OUT_DP)
