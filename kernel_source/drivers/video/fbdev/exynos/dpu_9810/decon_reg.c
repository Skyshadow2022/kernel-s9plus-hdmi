		...
		enum decon_set_trig en)
{
	...
	/*
	 * Stock CAL ignores SW_TRIG in VIDEO_MODE (MIPI TE path).
	 * DECON2/DP has no TE — must still accept SW_TRIG pulses or
	 * shadow updates never retire (black HDMI after bist=0).
	 */
...
int decon_reg_start(u32 id, struct decon_mode_info *psr)
{
	...
	/*
	 * Arm trigger before shadow update so DECON2 can enter RUN and
	 * retire SHADOW_REG_UPDATE_REQ (especially after BIST→live).
	 */
	if (psr->psr_mode == DECON_MIPI_COMMAND_MODE ||
	    psr->trig_mode == DECON_SW_TRIG ||
	    psr->out_type == DECON_OUT_DP)
	...
	/* keep trigger unmasked for ongoing frames */
	if (psr->psr_mode == DECON_MIPI_COMMAND_MODE ||
	    psr->trig_mode == DECON_SW_TRIG ||
	    psr->out_type == DECON_OUT_DP)
		decon_reg_set_trigger(id, psr, DECON_TRIG_ENABLE);
...
void decon_reg_set_winmap(u32 id, u32 win_idx, u32 color, u32 en)
{
	...
	/* Color Set — must use same decon id (DP uses decon2, not always 0) */
		...
		struct decon_window_regs *regs, u32 winmap_en)
{
	...
	if (win_en) {
		decon_dbg("%s: decon%d win id = %d\n", __func__, id, win_idx);
		/* Use decon `id` — hardcoded 0 broke DP/hdmi on decon2 */
		...
		decon_write(id, WIN_START_TIME_CONTROL(win_idx),
							regs->start_time);
	}
	...
	/* Colormap windows need no DMA channel — avoid dual-decon CH conflicts */
	if (win_en && !winmap_en)
		decon_reg_config_win_channel(id, win_idx, regs->type);

	decon_dbg("%s: regs->type(%d) winmap=%u\n", __func__, regs->type, winmap_en);
}
