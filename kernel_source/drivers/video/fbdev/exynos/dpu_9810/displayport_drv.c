...
static void displayport_kick_decon_live(struct displayport_device *displayport,
		bool cut_bist);
...
/**
 * Arm hybrid HW/SW/timer trigger so DECON2 keeps producing frames even if
 * DPIF TE is missing during BIST\to live cutover.
 */
static void displayport_arm_decon_trig(struct decon_device *decon)
{
	/* Full write — write_mask races with set_trigger and drops SW_TRIG_EN */
	decon_write(decon->id, HW_SW_TRIG_TIMER, 0x40000);
	decon_write(decon->id, HW_SW_TRIG_CONTROL,
			HW_TRIG_EN | SW_TRIG_EN | HW_SW_TRIG_TIMER_EN |
			HW_TRIG_ACTIVE_VALUE);
}

/* Keep DECON2 producing while live (DP TE often missing on PE) */
static struct delayed_work displayport_live_trig_work;

static void displayport_live_trig_fn(struct work_struct *work)
{
	struct displayport_device *displayport = get_displayport_drvdata();
	struct decon_device *decon = get_decon_drvdata(2);

	if (!displayport || !decon || !displayport->hpd_current_state ||
	    displayport->state != DISPLAYPORT_STATE_ON ||
	    displayport->bist_used)
		return;

	displayport_arm_decon_trig(decon);
	displayport_reg_set_stream_valid_force(1);
	if (displayport->dp_wq)
		queue_delayed_work(displayport->dp_wq,
				&displayport_live_trig_work, msecs_to_jiffies(16));
}
...
void displayport_live_trig_start(void)
{
	...
	if (!displayport || !displayport->dp_wq)
		return;
	mod_delayed_work(displayport->dp_wq, &displayport_live_trig_work, 0);
}
EXPORT_SYMBOL(displayport_live_trig_start);

void displayport_live_trig_stop(void)
{
	cancel_delayed_work_sync(&displayport_live_trig_work);
}
		...
		bool cut_bist)
{
	...
	/* Refresh lcd timing + HW_TRIG for current sink mode */
	decon_displayport_get_out_sd(decon);
	if (decon->dt.dft_win < MAX_DECON_WIN && decon->win[decon->dt.dft_win])
		fbinfo = decon->win[decon->dt.dft_win]->fbinfo;

	if (fbinfo && !IS_DECON_ON_STATE(decon))
		fb_blank(fbinfo, FB_BLANK_UNBLANK);

	if (!IS_DECON_ON_STATE(decon) && fbinfo == NULL) {
		displayport_err("kick_decon_live: no fbinfo and decon2 off\n");
		return;
	}
	...
	/*
	 * Soft-reset only when shadow is stuck — resetting a healthy RUN
	 * decon tears down DPIF and yields black after BIST cutover.
	 */
	shd = decon_read(decon->id, SHADOW_REG_UPDATE_REQ);
	if (shd) {
		decon_reg_set_trigger(decon->id, &psr, DECON_TRIG_ENABLE);
		displayport_arm_decon_trig(decon);
		if (decon_reg_wait_for_update_timeout(decon->id,
				SHADOW_UPDATE_TIMEOUT) < 0) {
			displayport_info("kick_decon_live: reset decon%d shd=0x%x\n",
					decon->id, shd);
	...
	} else if (!decon_reg_get_run_status(decon->id)) {
		decon_info("kick_decon_live: init+start decon%d\n", decon->id);
		decon_reg_init(decon->id, decon->dt.out_idx[0], &p);
	}

	displayport_arm_decon_trig(decon);
	if (decon_reg_start(decon->id, &psr) < 0)
		displayport_err("kick_decon_live: decon_reg_start failed\n");

	if (!IS_DECON_ON_STATE(decon))
		decon->state = DECON_STATE_ON;

	/* Blender BG + winmap both red (ARGB8888; alpha must be opaque) */
	decon_reg_set_blender_bg_image_color(decon->id, 0xff, 0x3ff, 0, 0);
	decon_set_color_window(decon, 0xFFFF0000);
	displayport_arm_decon_trig(decon);

	/* Let a few frames retire while BIST still owns the link */
	for (i = 0; i < 8; i++) {
		decon_write_mask(decon->id, HW_SW_TRIG_CONTROL, SW_TRIG_EN,
				SW_TRIG_EN);
		mdelay(8);
	}
	...
	if (cut_bist) {
		displayport->bist_used = 0;
		/* Reconfig live path while decon is already painting */
		displayport_enable(displayport);
		displayport_arm_decon_trig(decon);
		decon_set_color_window(decon, 0xFFFF0000);
		displayport_arm_decon_trig(decon);
		for (i = 0; i < 8; i++) {
			decon_write(decon->id, HW_SW_TRIG_CONTROL,
					HW_TRIG_EN | SW_TRIG_EN |
					HW_SW_TRIG_TIMER_EN | HW_TRIG_ACTIVE_VALUE);
			mdelay(8);
		}
		/*
		 * Stay on winmap red — DMA pan here caused continuous
		 * VIDEO FIFO_UNDER_FLOW (#32). Mirror/pan switches to DMA later
		 * with live_trig rearm feeding frames.
		 */
		displayport_reg_set_stream_valid_force(1);
		displayport_live_trig_start();
		displayport_info("kick_decon_live: after cut trig=0x%x lh=0x%x (winmap)\n",
				decon_read(decon->id, HW_SW_TRIG_CONTROL),
				displayport_read(SYSTEM_SST1_FUNCTION_ENABLE));
	}

	displayport_info("kick_decon_live: red %dx%d cut_bist=%d run=%u\n",
			decon->lcd_info->xres, decon->lcd_info->yres, cut_bist,
...
static int displayport_enable(struct displayport_device *displayport)
{
	...
	if (displayport->state == DISPLAYPORT_STATE_ON) {
		/*
		 * BIST already left state ON. Allow reconfigure into live
		 * video so decon unblank / bist=0 is not ignored.
		 */
		if (!displayport->bist_used) {
			u8 bpc_r = (u8)displayport->bpc;
			u8 dyn_r = (u8)displayport->dyn_range;

			displayport_info("%s reconfig live video: %s\n", __func__,
					supported_videos[displayport->cur_video].name);
			/*
			 * LH (DECON→DP) must be up BEFORE BIST is cut and
			 * VIDEO_EN samples the input — otherwise FIFO underflows
			 * and the sink stays black (BIST solid-red still works).
			 */
			displayport_reg_stop();
			displayport_reg_lh_p_ch_power(1);
			mdelay(5);
			displayport_write_mask(SST1_VIDEO_BIST_CONTROL, 0,
					BIST_EN | BIST_USER_DATA_EN);
	...
	/* Live (non-BIST) needs forced stream-valid until LH asserts it */
	if (!displayport->bist_used)
		displayport_reg_set_stream_valid_force(1);
		...
		const char *buf, size_t size)
{
	...
	case 0:
		/*
		 * Bring DECON2 up under BIST first, then cut BIST inside kick so
		 * the video FIFO is already fed (avoids black TV / underflow).
		 */
		displayport->bist_type = COLOR_BAR;
		if (displayport->state == DISPLAYPORT_STATE_ON) {
			if (displayport->best_video < supported_videos_pre_cnt)
			...
			displayport_kick_decon_live(displayport, true);
			displayport_info("bist OFF → live video %s\n",
					supported_videos[displayport->cur_video].name);
		} else {
			displayport->bist_used = 0;
		}
	...
	case 1:
		...
		if (displayport->state == DISPLAYPORT_STATE_ON) {
			...
			/*
			 * BIST is generated in the DP controller. Do not POWERDOWN
			 * decon2 — that tears DPIF and makes the next live cutover
			 * black. DP DMA is VGF1 so primary HWC keeps G0/G1.
			 * Mask FIFO under — empty DECON must not storm while BIST owns mux.
			 */
			displayport_live_trig_stop();
			displayport_reg_set_interrupt_mask(
					VIDEO_FIFO_UNDER_FLOW_MASK, 1);
			mutex_lock(&displayport->cmd_lock);
			displayport_reg_set_bist_video_configuration(
					displayport->cur_video, bpc,
					displayport->bist_type, dyn_range);
			displayport_reg_start();
			mutex_unlock(&displayport->cmd_lock);
			displayport_info("bist ON color-bar\n");
		}
		break;
	case 2:
		displayport->bist_used = 1;
	...
	case 4:
		/*
		 * Solid red via DP BIST user-data (no DECON). Control test:
		 * if TV shows red here, sink is fine and live path is the bug.
		 */
		displayport->bist_used = 1;
		displayport->bist_type = COLOR_BAR;
		if (displayport->state == DISPLAYPORT_STATE_ON) {
			...
			displayport_reg_set_video_configuration(
					displayport->cur_video, bpc, dyn_range);
			displayport_write(SST1_VIDEO_BIST_USER_DATA_R, 0xFF);
			displayport_write(SST1_VIDEO_BIST_USER_DATA_G, 0x00);
			displayport_write(SST1_VIDEO_BIST_USER_DATA_B, 0x00);
			displayport_write_mask(SST1_VIDEO_BIST_CONTROL, 1,
					BIST_USER_DATA_EN);
			displayport_reg_set_video_bist_mode(1);
			displayport_reg_start();
			mutex_unlock(&displayport->cmd_lock);
			displayport_info("bist ON solid red (user-data)\n");
		}
		break;
	case 11:
	case 12:
...
static ssize_t displayport_prefer_live_show(struct class *class,
		struct class_attribute *attr, char *buf)
{
	struct displayport_device *displayport = get_displayport_drvdata();

	return snprintf(buf, PAGE_SIZE, "%d\n", displayport->prefer_live);
}

static ssize_t displayport_prefer_live_store(struct class *dev,
		struct class_attribute *attr, const char *buf, size_t size)
{
	struct displayport_device *displayport = get_displayport_drvdata();
	int val = 0;
	...
	if (kstrtoint(buf, 10, &val))
		return size;
	displayport->prefer_live = val ? 1 : 0;
	displayport_info("prefer_live=%d (live via bist=0 / mirror)\n",
			displayport->prefer_live);
	/* Do not kick here — bist=0 owns the BIST→DECON cutover. */
	return size;
}
...
static int displayport_probe(struct platform_device *pdev)
{
	...
	INIT_DELAYED_WORK(&displayport_live_trig_work, displayport_live_trig_fn);
...
#endif
			/* Always expose bist toggle for AOSP DP bring-up */
