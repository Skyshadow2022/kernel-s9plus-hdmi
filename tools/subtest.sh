...
grep -q "cs47l92" <(ls /sys/bus/platform/devices/ 2>/dev/null) && P "audio: codec child" || W "audio: codec child device name not found"
...
grep -q sec_ts /proc/modules && P "touch: module loaded" || W "touch: builtin (module empty)"
l=$(dmesg | grep -c module_layout); [ "$l" = "0" ] && P "modules: no layout errors" || F "modules: $l layout errors"

# ---- display ----
grep -q "00000003" <(od -An -tx1 /proc/device-tree/decon_t@0x16050000/default_idma 2>/dev/null | tr -d ' ') && P "dtb: idma VG1" || F "dtb: idma wrong"
s=$(dumpsys SurfaceFlinger 2>/dev/null | grep -c "GLES"); [ "$s" -gt 0 ] && P "display: SF alive" || W "display: SF check inconclusive"

# ---- kernel core ----
grep -q "00000003" <(od -An -tx1 /proc/device-tree/decon_t@0x16050000/default_idma | tr -d ' ') && : || :
...
# ---- root ----
grep -q "uid=0" <(su -c id) && P "KernelSU: root works" || F "KernelSU: no root"
v=$(ksud debug version 2>/dev/null | grep -oE "[0-9]+"); [ "$v" -ge 33250 ] && P "KernelSU: version $v" || W "KernelSU: version $v"

# ---- radio/data ----
grep -q "wlan0" <(ip link) && P "wifi: wlan0 up" || F "wifi: no wlan0"
...
p=$(ps -A | grep -c "camera.*provider\|camera.provider"); [ "$p" -gt 0 ] && P "camera: provider running" || F "camera: no provider process"
